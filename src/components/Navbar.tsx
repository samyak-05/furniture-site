...
export default function Navbar() {
  ...
  const isElite = pathname === '/elite';
  const user = session?.user;
  const tag = user?.role || "customer";

  // Dynamic Greeting Generator for Admin
  useEffect(() => {
  ...
  return (
    <nav className={`fixed top-0 w-full z-[100] flex justify-between items-center px-4 md:px-12 py-3 md:py-4 transition-all duration-700 ${
      isElite ? 'bg-[#1a1c14] text-[#F5F5DC]' : 'bg-[#FAF9F6]/95 backdrop-blur-md text-black border-b border-black/5'
    }`}>
      ...
      <AnimatePresence>
        {isSearchOpen && (
          <motion.div 
            ...
            exit={{ y: -20, opacity: 0 }}
            className={`absolute inset-0 z-[110] flex items-center px-4 gap-4 ${isElite ? 'bg-[#1a1c14]' : 'bg-[#FAF9F6]'}`}
          >
            <div className="relative flex-1">
              <input
                ...
                className={`w-full py-2 pl-10 pr-4 rounded-full text-sm outline-none border ${
                  isElite ? 'bg-[#2a2e22] border-[#588157]/40 text-[#F5F5DC]' : 'bg-black/5 border-black/10'
                }`}
              />
      ...
      <div className="flex items-center gap-2 md:gap-8 shrink-0">
        <Link href="/" className={`text-xl md:text-2xl tracking-tighter shrink-0 ${isElite ? 'font-serif italic tracking-[0.2em]' : 'font-black'}`}>
          {tag === "admin" ? "WEBSITE" : isElite ? "ELITE" : "Classic"}
        </Link>

        {tag === "customer" && (
          <Link href={isElite ? "/classic" : "/elite"}>
            <motion.div 
              whileTap={{ scale: 0.95 }}
              className={`flex items-center gap-1 md:gap-2 px-2 md:px-3 py-1.5 rounded-full border transition-all cursor-pointer shadow-sm ${
                isElite ? 'border-[#588157]/40 text-[#A3B18A] hover:bg-[#588157] hover:text-[#F5F5DC]' : 'border-black/10 text-black hover:bg-black hover:text-white'
              }`}
            >
              <ArrowRightLeft size={10} strokeWidth={3} className="shrink-0" />
              <span className="text-[7px] font-black uppercase tracking-tighter sm:hidden">Classic</span>
              <span className="text-[9px] font-black uppercase tracking-widest hidden sm:inline">{isElite ? 'To CLASSIC' : 'To ELITE'}</span>
            </motion.div>
          </Link>
        )}
      </div>

      {/* CENTER: SEARCH BAR (Customers) OR DYNAMIC GREETING (Admin) */}
      <div className="hidden lg:flex flex-1 max-w-xl mx-8 justify-center">
        {tag === "customer" ? (
          <div className="relative w-full max-w-md">
            <input
              type="text"
              placeholder={`Search ${isElite ? 'Elite' : 'Classic'}...`}
            ...
            />
            <Search size={15} className={`absolute left-4 top-1/2 -translate-y-1/2 opacity-40 ${isElite ? 'text-[#F5F5DC]' : ''}`} />
          </div>
        ) : (
          /* ADMIN GREETING TEXT LAYER */
          <motion.div 
            initial={{ opacity: 0, y: -5 }}
            animate={{ opacity: 1, y: 0 }}
            className={`text-sm tracking-wider font-medium font-serif italic ${isElite ? 'text-[#F5F5DC]/90' : 'text-gray-800'}`}
          >
            {greeting}, <span className="font-sans not-italic font-black uppercase tracking-widest text-xs border-b pb-0.5 ml-1 border-current/20">{user?.name}</span>
          </motion.div>
        )}
      </div>
        ...
        {tag === "customer" && (
          <>
            ...
            <Link href="/cart" className="hidden sm:block p-2 relative transition-transform hover:scale-110">
              <ShoppingBag size={20} strokeWidth={isElite ? 1.5 : 2.5} />
              <span className={`absolute top-1 right-1 w-1.5 h-1.5 rounded-full ${isElite ? 'bg-[#588157]' : 'bg-black'}`} />
            </Link>
          </>
        )}

        {/* PROFILE DROPDOWN / SIGN IN ACTIONS */}
        {status === "loading" ? (
          <div className="w-8 h-8 rounded-full bg-current/5 animate-pulse" />
            ...
            <AnimatePresence>
              {isOpen && (
                <motion.div
                  ...
                  className={`absolute right-0 mt-4 w-56 md:w-64 overflow-hidden rounded-2xl shadow-2xl border ${isElite ? 'bg-[#2a2e22] border-[#588157]/30 text-[#F5F5DC]' : 'bg-white border-black/10 text-black'}`}
                >
                  <div className="px-5 py-4 border-b border-current/5 bg-current/5">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      {user?.role === "admin" && (
                        <span className="text-[8px] font-black uppercase tracking-widest px-2 py-0.5 bg-red-500 text-white rounded-full">Admin</span>
                      )}
                    </div>
                    <p className="text-[10px] opacity-40 truncate mt-0.5">{user?.email}</p>
                  </div>
                  ...
                  <div className="p-2 flex flex-col gap-1">
                    ...
                    {tag === "admin" && (
                      <Link href="/admin" className="flex items-center gap-3 px-4 py-2.5 text-xs font-bold uppercase hover:bg-current/5 rounded-xl transition-all"><User size={14} /> Admin Panel</Link>
                    )}
          ...
          <Link href="/signin" className={`px-5 md:px-8 py-2 md:py-2.5 rounded-full text-[9px] md:text-[10px] font-black tracking-widest uppercase transition-all shadow-lg ${
              isElite ? 'bg-[#588157] text-[#F5F5DC] hover:bg-[#344e41]' : 'bg-black text-white hover:bg-gray-800'
          }`}>Sign In</Link>
        )}
      </div>
    </nav>
